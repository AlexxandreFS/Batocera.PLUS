...
DATAINIT_DIR=/usr/share/batocera/datainit
...
if [ "$SYSTEM" == 'xbox' ]; then
   ...
   # executa o xemu
   exec /opt/Xemu/xemu_launcher.sh "${ROM}" "${P1GUID}" "${P2GUID}" "${P3GUID}" "${P4GUID}" "${RESOLUTION}" "${SCALING}" "${BOOTANIM}"
fi
...
if [ "$SYSTEM" == 'fpinball' ]; then
   ...
   exec /opt/Future_Pinball/fpinball_launcher.sh "${ROM}" "${RESOLUTION}" "${RATIO}" "${P1GUID}" "${P1NAME}"
fi
...
if [ "$SYSTEM" == 'vpinball' ]; then
   ...
   exec /opt/VisualPinball/vpinball.sh "${ROM}" "${RESOLUTION}" "${P1GUID}" "${CONTROLLER}" "${P1NAME}"
fi
...
if [ "$SYSTEM" == 'ps3' ]; then
    ...
    if [ ! -d "${HOME}/../saves/ps3" ] || [ ! "$(ls -l "${HOME}/configs/rpcs3" 2> /dev/null)" ]; then
        mkdir -p "${HOME}/../saves/ps3/GuiConfigs"
        ln -sf "${HOME}/../saves/ps3" "${HOME}/configs/rpcs3"
    fi
...
if [ "$SYSTEM" == 'windows' ] || [ "$SYSTEM" == 'mugen' ]; then
    ...
    if [ -d "${ROM}" ]; then
        ...
        if [ "${SYSTEM}" == 'mugen' ]; then
            GAME_SAVE_DIR="${SAVES_DIR}/${SYSTEM}/$(basename "${ROM}")"
        else
    ...
    if [ "${OPTION}" != 'auto' ]; then
        ...
        export VKBASALT_LOG_FILE="${HOME}/logs/vkBasalt.log"
        ...
        case ${OPTION} in
            ...
            user)
                ...
                # caso não seja achado é carregado uma config em "${HOME}/.config/vkBasalt/vkBasalt.conf"
    ...
    if [ "${ROM}" == "${MOUNT_DIR}" ]; then
        ...
        if ls -1A "${UPPER_DIR}" > /dev/null 2>&1; then
