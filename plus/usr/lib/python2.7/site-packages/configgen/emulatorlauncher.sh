...
function un7zip() {
    ...
    for ((i = 0; i != 2; i++)); do
        for j in $1; do
            if [ -d "$EXTRACT_DIR/$ROM_FILE" ]; then
                if [ -f "$EXTRACT_DIR/$ROM_FILE/$ROM_FILE$j" ]; then
                    ...
                    ROM="$EXTRACT_DIR/$ROM_FILE/$ROM_FILE$j"
                    return 0
                fi
            elif [ -f "$EXTRACT_DIR/$ROM_FILE$j" ]; then
                    for i in "$(grep -F "$(basename "$ROM")" "$BATOCERA_CONF" | sed "s/.$ROM_EXT./$j\"/")"; do
                        local ROM_NAME_ESC=$(sed 's/[^^]/[&]/g; s/\^/\\^/g' <<< "${ROM_FILE}$j")
                        sed -i "/$ROM_NAME_ESC/d" "$BATOCERA_CONF"
                        echo "$i" >> "$BATOCERA_CONF"
                    done
                ROM="$EXTRACT_DIR/$ROM_FILE$j"
                return 0
            fi
        done
        ...
        if [ "$(ls $EXTRACT_DIR)" ]; then
            rm -r "$EXTRACT_DIR/"*
        fi

        ### Descompacta a rom.
        7zr x "$ROM" "-o$EXTRACT_DIR" -bb0
    done
}
...
if [ "$SYSTEM" == 'neogeocd' ]; then
    case $EMULATOR in raine-cdz|raine-front-load|raine-top-load|raine-unibios|auto)
        case $CORE in raine2018|raine2015|auto)
...
if [ "$SYSTEM" == 'gamecube' ]; then
    ...
    ### FULLBOOT E FASTBOOT
    BIOS="$HOME/configs/dolphin-emu/Dolphin.ini"
    CONFIG="$HOME/batocera.conf"	
    BIOS_DIR="$HOME/../bios/GC"
    SAVE_DIR="$HOME/../saves/dolphin-emu/GC"

    for DIR in EUR USA JAP; do
        if ! [ -L "${SAVE_DIR}/${DIR}/IPL.bin" ]; then
            mkdir -p "${SAVE_DIR}/${DIR}"
            ln -s "${BIOS_DIR}/${DIR}/IPL.bin"  "${SAVE_DIR}/${DIR}/IPL.bin"
        fi
        ### FULLBOOT (Sempre Padrão)
        if [ "$(grep '^gamecube.fullboot=0' "$CONFIG")" ]; then
            sed -i 's/^SkipIPL.*/SkipIPL = True/' "$BIOS"
        elif [ "$(grep '^#gamecube.fullboot=0' "$CONFIG")" ]; then
            sed -i 's/^SkipIPL.*/SkipIPL = False/' "$BIOS"
        fi
    done
fi
...
if [ "$SYSTEM" == 'wii' ]; then
    ...
    ### FIX ASSINATURA DIGITAL (Sempre desativado)
    SIG="$HOME/configs/dolphin-emu/Dolphin.ini"
    if [ "$(grep '^EnableSignatureChecks = True' "$SIG")" ]; then
        sed -i 's/^EnableSignatureChecks.*/EnableSignatureChecks = False/' "$SIG"
    fi
fi
...
### Inclui compatibilidade a DLCs dos jogos de PSP que tem suporte a este recurso
if [ "$SYSTEM" == 'psp' ]; then
    STM="$HOME/configs/ppsspp/PSP/SYSTEM"
    if ! [ -d "$STM" ]; then
        GAME="$HOME/../saves/psp/PSP/GAME"
        CONF="$HOME/configs/ppsspp/PSP/GAME"
        DLC="$HOME/../roms/psp/DLC"
        mkdir -p "$(dirname "$STM")" "$(dirname "$GAME")"
        ln -s "$DLC" "$CONF"
        ln -s "$DLC" "$GAME"
    fi
fi
