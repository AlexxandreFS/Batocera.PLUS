...
case $SYSTEM in
    ...
    saturn)
        # Remova este código quando consertarem o bug destes cores não aceitarem roms no formato .cue
        case $CORE in
            yabasanshiro|kronos)
                un7zip '.ccd .CCD .chd .CHD .iso .ISO .bin .BIN' ;;
            *)
                un7zip '.ccd .CCD .cue .CUE .chd .CHD .iso .ISO .bin .BIN' ;;
        esac
        ;;
esac
...
if [ "$SYSTEM" == 'gamecube' ] || [ "$SYSTEM" == 'wii' ]; then
    ...
    if [ "$EMULATOR" == 'libretro' ]; then
		...
		DOLPHIN_GNR="/usr/lib/python2.7/site-packages/configgen/generators/dolphin/dolphinGenerator.py"
		...
		if [ "$(grep '^use.gamecube.pads=1' "$BATOCERA_CONF")" ] && [ "$(grep 'dolphinSettings.set("Core", "SIDevice0", "6")' "$DOLPHIN_GNR" )" ]; then
		    sed -i 's/dolphinSettings.set("Core", "SIDevice0", "6")/dolphinSettings.set("Core", "SIDevice0", "12")/' "$DOLPHIN_GNR"
			sed -i 's/dolphinSettings.set("Core", "SIDevice1", "6")/dolphinSettings.set("Core", "SIDevice1", "12")/' "$DOLPHIN_GNR"
			sed -i 's/dolphinSettings.set("Core", "SIDevice2", "6")/dolphinSettings.set("Core", "SIDevice2", "12")/' "$DOLPHIN_GNR"
			sed -i 's/dolphinSettings.set("Core", "SIDevice3", "6")/dolphinSettings.set("Core", "SIDevice3", "12")/' "$DOLPHIN_GNR"
		elif [ "$(grep '^#use.gamecube.pads=1' "$BATOCERA_CONF")" ] && [ "$(grep 'dolphinSettings.set("Core", "SIDevice0", "12")' "$DOLPHIN_GNR" )" ]; then
		    sed -i 's/dolphinSettings.set("Core", "SIDevice0", "12")/dolphinSettings.set("Core", "SIDevice0", "6")/' "$DOLPHIN_GNR"
			sed -i 's/dolphinSettings.set("Core", "SIDevice1", "12")/dolphinSettings.set("Core", "SIDevice1", "6")/' "$DOLPHIN_GNR"
			sed -i 's/dolphinSettings.set("Core", "SIDevice2", "12")/dolphinSettings.set("Core", "SIDevice2", "6")/' "$DOLPHIN_GNR"
			sed -i 's/dolphinSettings.set("Core", "SIDevice3", "12")/dolphinSettings.set("Core", "SIDevice3", "6")/' "$DOLPHIN_GNR"
		fi
	fi
fi
