...
if [ "$SYSTEM" == 'wiiu' ]; then
	...
	getOption otimizations
	OTIMIZATIONS="${OPTION}"
	
	#getOption showfps
	#SHOWFPS="${OPTION}"

    #exec /opt/Cemu/Cemu.sh "${ROM}" "${OTIMIZATIONS}" "${SHOWFPS}"
	exec /opt/Cemu/Cemu.sh "${ROM}" "${OTIMIZATIONS}"
...
if [ "$SYSTEM" == 'model2' ]; then
	...
	# Executa o Model 2
    exec /opt/Model2/Model2.sh "${ROM}" "${CORE}" "${RESOLUTION}" "${WIDESCREEN}" "${ANTIALIASING}" "${VSYNC}" "${ANISOTROPICFILTER}" "${DRAWCROSS}" "${RUMBLE}"
...
if [ "$SYSTEM" == 'model3' ]; then
    ...
    # Executa o Model 3
    exec /opt/Model3/Model3.sh "${ROM}" "${CORE}" "${RESOLUTION}" "${WIDESCREEN}" "${MULTITEXTURE}" "${MULTITHREAD}" "${QUADRENDERING}" "${VSYNC}" "${NEW3DENGINE}" "${THROTTLE}" "${STRETCH}" "${INTEGERSCALE}" "${POWERPCFREQUENCY}" "${RUMBLE}" "${EMULATENET}"
fi
...
if [ "$SYSTEM" == 'windows' ] || [ "$SYSTEM" == 'mugen' ]; then
    ...
    case ${OPTION} in
        ...
        fps)
            ...
            ;;
        *)
            echo "error, option ${OPTION} not supported to dxvk."
            exit 1
     esac
    ...
    if [ "${OPTION}" == 'on' ] && [ -e '/dev/input/js0' ]; then
        ...
        if [ -e '/dev/input/js0' ]; then
            while : ; do
                nice -n -20 xjoypad -device /dev/input/js0 \
    ...
    ### Captura a resolução da tela antes do jogo ser executado para restaura no final de seua execução ###
