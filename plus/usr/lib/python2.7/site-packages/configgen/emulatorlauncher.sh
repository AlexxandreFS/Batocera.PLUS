...
# Define a imagem do jogo se houver uma no diretório gamescreens/nome_do_sistema
if [ -d "$HOME/../gamescreens/$SYSTEM" ]; then
    # Verifica se o jogo esta compactado com 7z caso contrário IMG_GAME recebe o nome do jogo
	# -4 caracteres
	if [ "$(echo "$IMG_ROM" | grep '.7z$')" ] || [ "$(echo "$IMG_ROM" | grep '.7Z$')" ]; then
		IMG_GAME="$(basename "${ROM::-3}")"
	else 
	    IMG_GAME="$(basename "${ROM::-4}")"
    fi
    
	# Verifica se o jogo tem a extenção .dos ou .pc
    function imgdos {
	    IMG_ROM="$(basename "${ROM}")"
		if [ "$(echo "$IMG_ROM" | grep '.dos$')" ]; then
		    IMG_GAME="$(basename "${ROM::-4}")"
		elif [ "$(echo "$IMG_ROM" | grep '.pc$')" ] ; then
		    IMG_GAME="$(basename "${ROM::-3}")"
		fi
    }
	
	# Verifica se o jogo tem a extenção .scummvm, .residualvm ou esta
	# compactada com 7z, caso contrário IMG_GAME recebe o nome do jogo
	# -4 caracteres. A imagem precisa ter o mesmo nome do arquivo com a
	# extenção reconhecida pelo ES "Broken Sword/sword1.scummvm" logo
	# "gamescreen/nome_do_sistema/sword1.jpg" ou "gamescreen/nome_do_sistema/sword1.png"
	function imgscummvm {
	    IMG_ROM="$(basename "${ROM}")"
		IMG_GAME="$(basename "${ROM::-4}")"
		if [ "$(echo "$IMG_ROM" | grep '.scummvm$')" ]; then
		    IMG_GAME="$(basename "${ROM::-8}")"
		elif [ "$(echo "$IMG_ROM" | grep '.residualvm$')" ] ; then
		    IMG_GAME="$(basename "${ROM::-11}")"
		elif [ "$(echo "$IMG_ROM" | grep '.7z$')" ] ; then
		    IMG_GAME="$(basename "${ROM::-3}")"
		fi	    
	}
    
	# Verifica se o jogo tem a extenção .plus ou .PLUS
	function imgpc {
	    IMG_ROM="$(basename "${ROM}")"
		if [ "$(echo "$IMG_ROM" | grep '.plus$')" ] || [ "$(echo "$IMG_ROM" | grep '.PLUS$')" ]; then
		    IMG_GAME="$(basename "${ROM::-5}")"
		fi
	}
	
	# Nintendo Wii tem algumas particularidades com reconhecimento dos
	# jogos tipo nunchuk, wii remote na horizontal, etc... por uma limitação
    # de conhecimento meu o recurso gamescreen reconhece apenas os jogos
    # que não tem esta particularidade configurada na rom, mas isso pode mudar
	function imgwii {
	    IMG_ROM="$(basename "${ROM}")"
		IMG_GAME="$(basename "${ROM::-4}")"
		if [ "$(echo "$IMG_ROM" | grep '.ciso$')" ] || [ "$(echo "$IMG_ROM" | grep '.wbfs$')" ]; then
		    IMG_GAME="$(basename "${ROM::-5}")"
		elif [ "$(echo "$IMG_ROM" | grep '.7z$')" ] || [ "$(echo "$IMG_ROM" | grep '.7Z$')" ]; then
		    IMG_GAME="$(basename "${ROM::-3}")"
		fi
		
		for i in .si. .it. .in. .is. .ts. .ns. .st. .sn. .ti. .tn. .nt. .cc. ; do
		    if [ "$(echo "$IMG_ROM" | grep "${i}")" ]; then
		        if [ "$(echo "$IMG_ROM" | grep '.ciso$')" ] || [ "$(echo "$IMG_ROM" | grep '.wbfs$')" ]; then
		            IMG_GAME="$(basename "${ROM::-8}")"
				elif [ "$(echo "$IMG_ROM" | grep '.iso$')" ]; then
				    IMG_GAME="$(basename "${ROM::-7}")"
				elif [ "$(echo "$IMG_ROM" | grep '.7z$')" ] || [ "$(echo "$IMG_ROM" | grep '.7Z$')" ]; then
				    IMG_GAME="$(basename "${ROM::-6}")"
		        fi
    	    fi
		done
	}
	
	# Sistemas com particularidades especiais 
    case "$SYSTEM" in
        dos)     imgdos ;;
		scummvm) imgscummvm ;;
		windows) imgpc ;;
		linux)   imgpc ;; 
		wii)     imgwii ;;
		daphne)  IMG_GAME="$(basename "${ROM::-7}")" ;;
    esac
	
	# Se encontrado nome_do_jogo.png ou nome_do_jogo.iso na pasta "gamescreen/nome_do_sistema*
	# IMG_NAME recebe o caminho desta imagem
	if [ -f "$HOME/../gamescreens/$SYSTEM/$IMG_GAME.png" ] || [ -f "$HOME/../gamescreens/$SYSTEM/$IMG_GAME.jpg" ] ; then
        for FILE in "$IMG_GAME.png" "$IMG_GAME.jpg"; do
            if [ -f "$HOME/../gamescreens/$SYSTEM/${FILE}" ]; then
                IMG_NAME="$HOME/../gamescreens/$SYSTEM/${FILE}"
	        fi
        done
    fi
	
	if [ "$SYSTEM" == 'gamecube' ] && [ "$ROM" == '/userdata/roms/gamecube/batocera.plus' ]; then
	    exit 0
    fi
...
if [ "${SYSTEM}" == 'n64dd' ] || [ "${SYSTEM}" == 'n64' ]; then
    ...
    RETROARCH_CORE_CFG="$HOME/.config/retroarch/config/ParaLLEl N64/ParaLLEl N64.opt"
...
if [ "$SYSTEM" == 'gamecube' ] || [ "$SYSTEM" == 'wii' ]; then
    ...
    if [ "$EMULATOR" == 'libretro' ]; then
        ...
        if [ "${CORE}" == 'dolphin-user-config' ] || [ "${ROM}" == '/userdata/roms/gamecube/batocera.plus' ]; then
...
#        nice -n 20 xjoykill -hotkey "${BOTAO_HOTKEY}" -start "${BOTAO_START}" -kill /opt/Wine/killwine &
#    fi

#    unset BOTOES BOTAO_HOTKEY BOTAO_START

#    exec /opt/Cemu/Cemu.sh "$ROM" "$CORE"
...
if [ "$SYSTEM" == 'windows' ] || [ "$SYSTEM" == 'mugen' ]; then
    ...
    if [ -f 'batocera.plus' ]; then
        WINE_EXE='batocera.plus'

    elif [ -f 'batocera.plus.exe' ]; then
        WINE_EXE='batocera.plus.exe'

    elif [ "$(ls -1 | grep -i '.plus.exe$')" ]; then
        WINE_EXE="$(ls -1 | grep -i '.plus.exe$' | tail -n 1)"

    elif [ "$(ls -1 | grep -i '.exe$')" ]; then
        WINE_EXE="$(ls -1 | grep -i '.exe$' | tail -n 1)"

    elif [ "$(find . | grep -i 'batocera.plus.exe$')" ]; then
        WINE_EXE="$(find . | grep -i 'batocera.plus.exe$' | tail -n 1)"
        cd "$(dirname "${WINE_EXE}")"
        WINE_EXE="$(basename "${WINE_EXE}")"

    elif [ "$(find . | grep -i '.plus.exe$')" ]; then
        WINE_EXE="$(find . | grep -i '.plus.exe$' | tail -n 1)"
        cd "$(dirname "${WINE_EXE}")"
        WINE_EXE="$(basename "${WINE_EXE}")"

    elif [ "$(find . | grep -i ".exe$")" ]; then
        WINE_EXE="$(find . | grep -i '.exe$' | tail -n 1)"
        cd "$(dirname "${WINE_EXE}")"
        WINE_EXE="$(basename "${WINE_EXE}")"
    fi
    ...
    if [ "${WINE_EXE}" == 'batocera.plus' ]; then
        ...
        if [ "${WIN_SAVE_DIR}" ]; then
            ...
            WIN_SAVE_DIR="${SHARE}/system/configs/wine/drive_c/${WIN_SAVE_DIR:3}"
    ...
    if [ "${WINE_EXE}" == 'batocera.plus' ]; then
        ...
        if [ "$(dos2unix --info=c "${WINE_EXE}")" ]; then
            dos2unix "${WINE_EXE}"
        fi
        ...
        if [ "${EMULATOR}" == 'wine-staging' ]; then
            case ${CORE} in
                ...
                wine32)
                    echo 'Em breve...'
                    #WINEPREFIX='/userdata/system/configs/wine/wine32' WINEARCH='win32' wine "${WINE_EXE}" ;;
            esac
        else
            wine "${WINE_EXE}"
        fi
