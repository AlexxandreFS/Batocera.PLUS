...
if [ "$SYSTEM" == 'windows' ] || [ "$SYSTEM" == 'mugen' ]; then
    ...
    if [ "${OPTION}" != auto ]; then
       ...
       OPT='clarity reshade simple gaussian game'
       for i in $OPT; do
            if [ "${OPTION}" == "${i}" ]; then
               rm -rf "${HOME}/.config/vkBasalt/vkBasalt.conf"
               break
            fi
       done
    ...
    case ${OPTION} in
        clarity)
            export VKBASALT_CONFIG_FILE=/opt/Wine/apps/vkbasalt/vkBasalt.conf.clarity
            ;;
        reshade)
            export VKBASALT_CONFIG_FILE=/opt/Wine/apps/vkbasalt/vkBasalt.conf.reshade
            ;;
        simple)
            export VKBASALT_CONFIG_FILE=/opt/Wine/apps/vkbasalt/vkBasalt.conf.simple
            ;;
        gaussian)
            export VKBASALT_CONFIG_FILE=/opt/Wine/apps/vkbasalt/vkBasalt.conf.gaussian
            ;;
        goverlay)
            cp -rf "${HOME}/.config/goverlay/vkBasalt.conf" "${HOME}/.config/vkBasalt/vkBasalt.conf"
