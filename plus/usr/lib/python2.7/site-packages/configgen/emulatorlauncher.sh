...
if [ "${EMULATOR}" == 'libretro' ]; then
    if [ "${CORE}" == 'kronos' ] || [ "${CORE}" == 'dolphin' ]; then
        VIDEO_DRIVER='glcore'
    fi
fi
...
if [ "$SYSTEM" == 'wiiu' ]; then
	...
	exec /opt/Cemu/Cemu.sh "${ROM}" "${OTIMIZATIONS}" "${RENDER}" "${SYNC}" "${DXVK}" "${SHOWFPS}" "${MOUSE}" "${P1GUID}"
...
if [ "$SYSTEM" == 'windows' ] || [ "$SYSTEM" == 'mugen' ]; then
    ...
    case ${OPTION} in
        ...
        auto)
            ;;
        *)
            echo "error, option ${OPTION} not supported to sync."
            exit 1
    esac
    ...
    if [ "${OPTION}" == 'auto' ]; then
        ...
        WINE_RESOLUTION="explorer /desktop=Batocera.PLUS,${OPTION} "
    fi
    ...
    case "${GAME_EXE}" in
        batocera.plus)
            ...
            export CORE
            eval ./batocera.plus "${WINE_RESOLUTION}$@"
            ;;
        explorer)
            eval ${CORE}${OPTION} "${WINE_RESOLUTION}${GAME_EXE}" "z:/userdata/roms/windows/${ROM}"
            ;;
        *)
            eval ${CORE}${OPTION} "${WINE_RESOLUTION}${GAME_EXE}"
            ;;
    esac
