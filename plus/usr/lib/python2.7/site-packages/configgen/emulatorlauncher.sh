...
if [ "${EMULATOR}" == 'flycast' ] && [ "${CORE}" == 'flycast' ]; then
    # Adiciona compatibilidade com os saves (VMU) entre o flycast standalone
    # e a versão libretro.

    if ! [ -L "${SAVES_DIR}/dreamcast/flycast" ]; then
        mkdir -p   "${SAVES_DIR}/dreamcast/flycast"
        rm    -r   "${SAVES_DIR}/dreamcast/flycast"
        ln    -s . "${SAVES_DIR}/dreamcast/flycast"
    fi

    # Ativa por padrão 4 VMUs na versão standalone para ficar igual
    # a versão libretro.

    if ! [ -f "${HOME}/config/flycast/emu.cfg" ]; then
        mkdir -p "${HOME}/config/flycast"

        echo '[input]' > "${HOME}/config/flycast/emu.cfg"

        for INDEX in {1..4}; do
            echo "device${INDEX} = 0"    >> "${HOME}/config/flycast/emu.cfg"
            echo "device${INDEX}.1 = 1"  >> "${HOME}/config/flycast/emu.cfg"
            echo "device${INDEX}.2 = 10" >> "${HOME}/config/flycast/emu.cfg"
        done
    fi
fi
...
if [ "${SYSTEM}" == 'switch' ]; then
   ...
   if [ "${CORE}" == 'publish' ]; then
        xjoykiller /opt/Ryujinx/killryujinx "${P1INDEX}" "${P1GUID}"
        /opt/Ryujinx/ryujinx.sh "${ROM}" "${P1GUID}" "${RENDER}"
   else
        xjoykiller /opt/Yuzu/killyuzu  "${P1INDEX}" "${P1GUID}"
        /opt/Yuzu/yuzu.sh "${ROM}" "${CORE}" "${P1GUID}" "${RENDER}"
   fi
...
if [ "$SYSTEM" == 'model2' ]; then
    ...
    xjoykiller /usr/bin/killwine "${P1INDEX}" "${P1GUID}"

    # Executa o Model 2
    /opt/Model2/Model2.sh \
...
if [ "${SYSTEM}" == 'openbor' ]; then
    xjoykiller /opt/OpenBOR/killfopenbor.sh "${P1INDEX}" "${P1GUID}"
    /opt/OpenBOR/OpenBOR.sh "${CORE}" "${ROM}"
    xjoykiller --exit
    exit 0
fi
...
if [ "${SYSTEM}" == 'j2me' ]; then
    ...
    if [ "${EMULATOR}" == 'freej2me' ] || [ "${EMULATOR}" == 'auto' ]; then
        xjoykiller /opt/Freej2me/killfreej2me.sh "${P1INDEX}" "${P1GUID}"
        /opt/Freej2me/freej2me.sh "${ROM}"
        xjoykiller --exit
        exit 0
    fi
fi
