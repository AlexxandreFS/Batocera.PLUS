...
if [ "$SYSTEM" == 'wiiu' ]; then
    ...
    # executa o cemu
    exec /opt/Cemu/Cemu.sh "${ROM}" "${RENDER}" "${SYNC}" "${SHOWFPS}" "${MOUSE}" "${P1GUID}" "${INTEL}"
fi
...
if [ "${SYSTEM}" == 'switch' ]; then
   ...
   exec /opt/Yuzu/yuzu.sh "${ROM}" "${CORE}" "${P1GUID}" "${MOUSE}"
fi
...
if [ "$SYSTEM" == 'ps3' ]; then
   if [ ! "$(ls -A "${HOME}/configs/rpcs3/dev_flash/vsh/etc" 2> /dev/null)" ]; then
...
if [ "$SYSTEM" == 'windows' ] || [ "$SYSTEM" == 'mugen' ]; then
    ...
    # DXVK Show FPS
    # Este código deve ser executado sempre depois do bloco do DirectX Engine.
    if [ "${MANGOHUD}" == "2" ]; then
        let DXVK=${DXVK}+1
        export DXVK
    fi
