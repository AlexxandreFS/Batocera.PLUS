...
if [ "${OPTION}" != 'auto' ]; then
    export MANGOHUD_CMD='/opt/MangoHud/mangohud --dlsym'
...
if [ "${OPTION}" != 'auto' ]; then
    ...
    case ${OPTION} in
        default) export VKBASALT_CONFIG_FILE=/opt/vkBasalt/profiles/vkBasalt.conf.default   ;;
        clarity) export VKBASALT_CONFIG_FILE=/opt/vkBasalt/profiles/vkBasalt.conf.clarity   ;;
        fakehdr) export VKBASALT_CONFIG_FILE=/opt/vkBasalt/profiles/vkBasalt.conf.fakehdr   ;;
      fakehdrv2) export VKBASALT_CONFIG_FILE=/opt/vkBasalt/profiles/vkBasalt.conf.fakehdrv2 ;;
       gaussian) export VKBASALT_CONFIG_FILE=/opt/vkBasalt/profiles/vkBasalt.conf.gaussian  ;;
        reshade) export VKBASALT_CONFIG_FILE=/opt/vkBasalt/profiles/vkBasalt.conf.reshade   ;;
           user) # Carrega uma config "vkBasalt.conf" na pasta do jogo
...
if [ "$SYSTEM" == 'wiiu' ]; then
    ...
    # executa o cemu
    exec /opt/Cemu/Cemu.sh "${ROM}" "${RENDER}" "${SYNC}" "${INTEL}"
...
if [ "$SYSTEM" == 'vpinball' ]; then
    ...
    exec /opt/VisualPinball/vpinball.sh \
        "${ROM}" "${RESOLUTION}" "${P1GUID}" "${CONTROLLER}" "${P1NAME}"
...
if [ "$SYSTEM" == 'ps3' ]; then
    ...
    if [ ! "$(ls -A "${HOME}/configs/rpcs3/dev_flash/vsh/etc" 2> /dev/null)" ] && [ "${CORE}" != 'rpcs3-mainline' ]; then
        /usr/bin/batocera-config-rpcs3 '--firmware'
    elif [ "${CORE}" == 'rpcs3-mainline' ]; then
        exec /opt/Rpcs3/rpcs3.sh "${ROM}" "${P1GUID}"
    fi
