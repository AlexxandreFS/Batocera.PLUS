        ...
        case ${CORE} in
            pcsx2-mainline)
                ...
                exec /opt/Pcsx2/pcsx2.sh \
                    "${CORE}"     "${P1GUID}" "${ROM}"    "${RESOLUTION}" "${WIDESCREEN}" \
                    "${BOOTANIM}" "${I_RES}"  "${A_FILT}" "${WSCRH}" "${SPEEDHACKS}"
                ;;
            pcsx2-legacy)
                exec /opt/Pcsx2/pcsx2.sh "${CORE}" "${P1GUID}" "${ROM}"
                ;;
            pcsx2)
                ;;
        esac
...
if [ "${SYSTEM}" == 'windows' ] || [ "${SYSTEM}" == 'mugen' ]; then
    ...
    exec /opt/Wine/wine-emulationstation.sh \
        ...
        "${MANGOHUD}"       "${VKBASALT}"  "${OPTIMIZATIONS}" "${INGAME_VIDEOS}" \
        "${EMULATE_JOYPAD}" "${VIDEOMODE}"
fi
