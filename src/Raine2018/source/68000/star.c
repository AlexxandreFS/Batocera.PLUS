 ...
 *          bad code, but anyway...). So this version adds at least the top
 *          limit check (I hope the bottom limit will never be necessary!!!)
 */
#endif
...
enum eamode {
  ...
  aind, /* Address Register Indirect */
  ainc, /* Address Register Indirect with Post-Increment */
  adec, /* Address Register Indirect with Pre-Decrement */
  adsp, /* Address Register Indirect with Displacement */
  axdp, /* Address Register Indirect with Index */
  absw, /* Absolute Short */
  absl, /* Absolute Long  */
  pcdp, /* Program Counter with Displacement */
  pcxd, /* Program Counter with Index */
  immd /* Immediate */
};
...
*/
	...
	emit("push edi\n");
	emit("push esi\n");
	if(use_stack) emit("mov edi,[esp+16]\n");
	else          emit("mov edi,eax\n");
	emit("mov esi,contextbegin\n");
	emit("mov ecx,(contextend-contextbegin)/4\n");
	emit("rep movsd\n");
	/*
	emit("%%assign i 0\n");
...
*/
	...
	emit("push esi\n");
	emit("push edi\n");
	emit("mov edi,contextbegin\n");
	emit("mov ecx,(contextend-contextbegin)/4\n");
	if(use_stack) emit("mov esi,[esp+16]\n");
...
static void align_stack(char *reg1, char *reg2) {
  emit("     push   ebp\n");
  emit("     mov    ebp,esp\n");
  emit("     sub    esp,32     ; stack aligned on 32\n");
  emit("     and    esp,0xffffffc0 \n");
  emit("     mov    [esp],%s \n",reg1);
  if (reg2)
    emit("     mov    [esp+4],%s \n",reg2);
}
...
static void pcrel_adjust(int size) {
  /* This should be called instead of "call readmemory" for pc relative modes */
  /* Since it's pc relative, it does not need to read memory map, just fetch from
     the program rom, and it does not even need to check the offset in the fetch
     table (pc relative -> in the same page normally !) */
  ...
  emit("add edx, ebp\n");
  if (size == 1) {
    // byte swap (only for byte size)
    emit("xor edx,1\n");
    emit("movsx ecx,byte[edx]\n");
  } else if (size == 2) {
    emit("movsx ecx,word[edx]\n");
  } else if (size == 4) {
    emit("movsx ecx,word[edx]\n");
    emit("shl   ecx,16\n");
    emit("mov   cx,word[edx+2]\n");
  } else {
    fprintf(stderr,"size %d\n",size);
    exit(1);
  }
  emit(".finlong:\n");
  /* Don't forget to put emit("sub edx, ebp") if you need edx after this !!! */
}

static void pcrel_restore_dx(int size) {
  if (size == 1)
...
static void ea_step_read_signword(enum eamode mode, int reg) {
	...
	case absw: case absl:
		...
		break;
	case pcdp: case pcxd:
	  // emit(".infinite_2: jmp short .infinite_2\n");
          emit("add edx, ebp\n");
		emit("movsx ecx,word[edx]\n");
		break;
	case immd:
...
static void i_movem_control(void){
	...
	if(main_dr==0){/*register->memory*/
		...
		if (main_eamode == pcdp || main_eamode == pcxd) {
		  pcrel_adjust(main_size);
		  pcrel_restore_dx(main_size);
		} else
		  emit("call readmemory%s\n",sizename[main_size]);
