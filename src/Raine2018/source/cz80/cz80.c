...
uint8_t FASTCALL Cz80_Read_Dummy(uint32_t ctx, const uint16_t adr);
void FASTCALL Cz80_Write_Dummy(uint32_t ctx, const uint16_t adr, uint8_t data);

uint8_t FASTCALL Cz80_Interrupt_Ack_Dummy(uint32_t ctx, uint8_t param);
