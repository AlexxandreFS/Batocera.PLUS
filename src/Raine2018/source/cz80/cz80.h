...
typedef uint8_t FASTCALL CZ80_READ(uint32_t ctx, uint16_t adr);
typedef void FASTCALL CZ80_WRITE(uint32_t ctx, uint16_t adr, uint8_t data);

#if CZ80_USE_WORD_HANDLER
typedef uint16_t FASTCALL CZ80_READ_WORD(uint32_t ctx, uint16_t adr);
typedef void FASTCALL CZ80_WRITE_WORD(uint32_t ctx, uint16_t adr, uint16_t data);
#endif

typedef void FASTCALL CZ80_RETI_CALLBACK(uint32_t ctx);
typedef uint8_t FASTCALL CZ80_INT_CALLBACK(uint32_t ctx, uint8_t param);
...
void    Cz80_Set_Ctx(cz80_struc *cpu, uint32_t ctx);
