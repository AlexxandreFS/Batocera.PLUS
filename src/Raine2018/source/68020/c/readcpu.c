...
static int u020_strncmp(const char *a, const char *b, int len)
{
	int i;
	for(i=0;i<len;i++){
		if(a[i] != b[i])
...
static void u020_abort(void)
{
}

static int u020_isspace(char a)
{
	if((a == 0x20) || (a == 0x0D) || (a == 0x0A) || (a == 0x09))
		return 1;
	else
		return 0;
}

static DEF_INLINE amodes mode_from_str (const char *str)
{
    if (! u020_strncmp (str, "Dreg", 4)) return Dreg;
    if (! u020_strncmp (str, "Areg", 4)) return Areg;
    if (! u020_strncmp (str, "Aind", 4)) return Aind;
    if (! u020_strncmp (str, "Apdi", 4)) return Apdi;
    if (! u020_strncmp (str, "Aipi", 4)) return Aipi;
    if (! u020_strncmp (str, "Ad16", 4)) return Ad16;
    if (! u020_strncmp (str, "Ad8r", 4)) return Ad8r;
    if (! u020_strncmp (str, "absw", 4)) return absw;
    if (! u020_strncmp (str, "absl", 4)) return absl;
    if (! u020_strncmp (str, "PC16", 4)) return PC16;
    if (! u020_strncmp (str, "PC8r", 4)) return PC8r;
    if (! u020_strncmp (str, "Immd", 4)) return imm;
    u020_abort ();
    return(0);
}

static DEF_INLINE amodes mode_from_mr (int mode, int reg)
{
    switch (mode) {
	case 0: return Dreg;
	case 1: return Areg;
...
out2:
    ...
    for (variants = 0; variants < (1 << id.n_variable); variants++) {
		...
		pos = 0;
		while (opcstr[pos] && !u020_isspace(opcstr[pos])) {
			if (opcstr[pos] == '.') {
				pos++;
				switch (opcstr[pos]) {
		...
		case 'D':
			...
			case 'A':
				...
				case '#':
					...
					case 'i': srcmode = immi; srcreg = (INT32)(INT8)bitval[biti];
						if (CPU_EMU_SIZE < 4) {
	...
	case 'D':
		...
		case 'A':
			...
			case '#':
				...
				case 'i': destmode = immi; destreg = (INT32)(INT8)bitval[biti]; break;
...
endofline:
	...
	if (find == -1) {
		for (find = 0;; find++) {
			...
			if (strlen(lookuptab[find].name) == 0) u020_abort();
		}
	}
	else {
		table68k[opc].mnemo = lookuptab[find].mnemo;
