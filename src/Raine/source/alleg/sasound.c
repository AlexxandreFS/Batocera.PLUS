...
//
// the sdl sound driver is now the main sound driver. This one is maintained for the
// dos version but it might lack some features in the long run.

// dx samples : their status can't be saved in savegames with allegro right now since
// we don't control anything in their playback (the code is easier but there's no
// control at all). I guess we could backport what I did for sdl to be able to do that
// but I don't think it's worth the trouble to do it.
