...
static void timer_callback_a (int n)
{
	...
	chip->timer_A = timer_adjust( chip->timer_A_time[ chip->timer_A_index ], n, 0,timer_callback_a);
	chip->timer_A_index_old = chip->timer_A_index;
	if (chip->irq_enable & 0x04)
	{
		int oldstate = chip->status & 3;
		chip->status |= 1;
		if ((!oldstate) && (chip->irqhandler)) {
		  (*chip->irqhandler)(1);
		}
	}
...
static void timer_callback_b (int n)
{
	...
	chip->timer_B = timer_adjust( chip->timer_B_time[ chip->timer_B_index ], n, 0, timer_callback_b);
...
int YM2151Init(int num, int clock, int rate)
{
	...
	ym2151_state_save_register( YMPSG, YMNumChips );
	...
	*/
	...
	do
	{
		if (op->pms)	/* only when phase modulation from LFO is enabled for this channel */
		{
			...
			if (mod_ind)
			{
				INT32 kc_channel =	op->kc_i + mod_ind;
				if (kc_channel >= 0) {
