...
int MSM5232_sh_start (const struct MSM5232interface *intf)
{
	int i,chip;
	int vol[2];
	char buf[2][40];
	...
	if ( msm5232_init(intf, audio_sample_rate) != 0)
		return 1;
	...
	for (chip=0; chip < intf->num; chip++)
	{
		for (i=0; i<=1; i++)
		{
			...
			name[i] = buf[i];
			sprintf(buf[i],"%s #%d Group %c","msm5232",chip,'1'+i);
		}
		stream[chip] = stream_init_multim(2,name,vol,audio_sample_rate,chip,MSM5232_update_one);
	}
	return 0;
}
...
WRITE_HANDLER ( MSM5232_0_w )
{
	...
	msm5232_write(0, offset & 0xf, data);
}

WRITE_HANDLER ( MSM5232_1_w )
{
	// stream_update (stream[1], 0);
	msm5232_write(1, offset & 0xf, data);
}
