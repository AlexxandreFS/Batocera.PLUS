 ...
 *   by Aaron Giles
 * Adaptation from Mame. Only difference (known) : no accum_mask.
 * Ah, and mostly don't care about lsb and msb (reg_read/write functions)
 **********************************************************************************************/
...
int ES5506_sh_start(const struct ES5506interface *intf)
{
	...
	for (i = 0; i < intf->num; i++)
	{
		...
		es5506[i].region_base[0] = (intf->region0[i] ? (UINT16 *)load_region[intf->region0[i]] : (UINT16*)PCMROM);
		es5506[i].region_base[1] = (intf->region1[i] ? (UINT16 *)load_region[intf->region1[i]] : (UINT16*)PCMROM);
		es5506[i].region_base[2] = (intf->region2[i] ? (UINT16 *)load_region[intf->region2[i]] : (UINT16*)PCMROM);
		es5506[i].region_base[3] = (intf->region3[i] ? (UINT16 *)load_region[intf->region3[i]] : (UINT16*)PCMROM);
...
READ16_HANDLER( ES5506_data_0_word_r )
{
  ...
  offset&=0x7f;
  offset >>= 1;
  res = es5506_reg_read(&es5506[0], offset);
  return res;
}
