...
extern UINT8 z80_readop(UINT32 adr);
extern UINT8 z80_readop_arg(UINT32 adr);
extern UINT16 z80_readop_arg16(UINT32 adr);
extern UINT8 z80_read8(UINT32 adr);
extern UINT16 z80_read16(UINT32 adr);
extern void z80_write8(UINT32 adr,UINT8 data);
extern void z80_write16(UINT32 adr,UINT16 data);
extern UINT8 z80_read_port(UINT32 adr);
extern void z80_write_port(UINT32 adr,UINT8 data);
UINT8 ExitOnEI;
UINT8 Z80Has16bitsPorts; // just for compatibility with mz80 but ignored
 ...
 ***************************************************************/
#define IN(port)   z80_read_port(port)

/***************************************************************
 * Output a byte to given I/O port
 ***************************************************************/
#define OUT(port,value) z80_write_port(port,value)

/***************************************************************
 * Read a byte from given memory location
 ***************************************************************/
#define RM(addr) z80_read8(addr)
 ...
 ***************************************************************/
#define WM(addr,value) z80_write8(addr,value)
...
INLINE UINT32 ARG16(void)
{
	...
	PC += 2;
#ifdef LSB_FIRST
	return z80_readop_arg16(pc);
#else
	return z80_readop_arg(pc) | (z80_readop_arg((pc+1)&0xffff) << 8);
#endif
}
...
OP(illegal,1) {
	logerror("Z80 ill. opcode $%02x $%02x\n",
		z80_readop((PCD-1)&0xffff), z80_readop(PCD));
}
...
{
    if (!Z80.z80Base) return 0; // for the fake z80 (truxton2 for example)
	z80_ICount = cycles;
	...
	dwElapsedTicks += cycles - z80_ICount;
...
{
    Z80.dwElapsedTicks = dwElapsedTicks;
	if( dst )
		*(Z80_Regs*)dst = Z80;
}
...
void z80_set_context (void *src)
{
	...
	Z80 = *(Z80_Regs*)src;
    change_pc(PCD);
    dwElapsedTicks = Z80.dwElapsedTicks;
}
